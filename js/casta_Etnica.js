...
(async function () {
    ...
    const DATA_URL = "js/casta_equiment/data/casta_Etnica.json"; // ← đổi sang file JSON của bạn
    ...
    const normalize = (p) => {
        ...
        const weight = s.weight ?? s.peso ?? null;
        ...
        // Dòng 2: các thông số còn lại (để dùng cho inquiry / fallback)
        const detailParts = [];
        ...
        return {
            ...
            line2: detailStr,
            label: "",
            price: null,          // CASTA không có giá
            currency: "",
            image: p?.image || "img/placeholder.webp",
            href: p?.url || "#",  // chỉ dùng để gửi email
            sp: null,
            specs: {
                width,
                depth,
