...
// Load JSON Combisteel + render product grid + filter giống panel Magento Bakery
...
(async function () {
    ...
    const DATA_URL = "js/data/Combisteel_bakery_bakery_refrigeration.json";
    ...
    function normalize(raw) {
        ...
        return {
            ...
            voltage: txt(s.voltage),                          // 230, 400...

            // field riêng cho filter refrigeration
            insulation_thickness: txt(s.insulation_thickness),
            energy_label: txt(s.energy_label),                // B, C, D, E, F, No energy label required
            defrost: txt(s.defrost),                          // Automatic
            mobile: txt(s.mobile),                            // Yes / No
            lockable: txt(s.lockable),                        // Yes
            type_of_cooling: txt(s.type_of_cooling),          // Ventilated
            cooling_agent: txt(s.cooling_agent),              // R 290, R 600 A
            lights: txt(s.lights),                            // Yes / Yes, LED
            operation: txt(s.operation),                      // Digital
            color: txt(s.color),                              // Black, Black / Stainless steel...
            ...
            // CATEGORY – bạn nhớ map cho đúng trong JSON (vd: Bakery cabinets / Bakery counters...)
            category: txt(
                raw.category ||
                raw.category_name ||
                raw.category_label ||
    ...
    // ===== FILTER CONFIG: GIỐNG PANEL BAKERY =====
    const FILTER_CONFIG = {
        // === CATEGORY ===
        category: {
            label: "Category",
            ...
            type: "value",
            buckets: [
                { id: "cat-bakery-cabinets", label: "Bakery cabinets", value: "Bakery cabinets" },
                { id: "cat-bakery-counters", label: "Bakery counters", value: "Bakery counters" },
                { id: "cat-pastry-displays", label: "Pastry displays", value: "Pastry displays" },
                {
                    id: "cat-accessories-bakery-refrigeration",
                    label: "Accessories for bakery refrigeration",
                    value: "Accessories for bakery refrigeration",
                },
            ],
        },
        ...
        price: {
            ...
            buckets: [
                ...
                { id: "p-6000+", label: "6,000.00 and above", min: 6000, max: Infinity },
            ],
        },
    ...
    function buildFilterPanel() {
        // Fill dynamic CATEGORY buckets từ allProducts
        const catConf = FILTER_CONFIG.category;
        if (catConf) {
            const catMap = new Map();
            allProducts.forEach((p) => {
                const name = p.category && p.category.trim() ? p.category.trim() : "Other";
                catMap.set(name, (catMap.get(name) || 0) + 1);
            });

            catConf.buckets = Array.from(catMap.keys())
                .sort()
                .map((name, idx) => ({
                    id: `cat-${idx}`,
                    label: name,
            ...
            .map(([groupId, conf]) => {
                const bucketHtml = conf.buckets
                    .map((b) => {
                        ...
                        if (!count) return ""; // ẩn bucket không có sản phẩm
              ...
              `;
                ...
                // block filter giống structure bạn gửi (hover:text-bakery)
                return `
          <div class="filter-option mb-3 pb-3 border-black border-b" x-data="{ open: false }">
            <div class="filter-options-title flex justify-between items-center cursor-pointer text-normal hover:text-bakery"
                 :class="{ 'pb-4': open }"
                 @click="open = !open">
