...
(async function () {
  ...
  const DATA_URL = "js/data/unox_trays_all_categories.json"; // file tổng 110 sản phẩm
  ...
  function renderProducts() {
    ...
    attachCardHandlers();          // <-- gắn sự kiện mở popup
  ...
  function buildCategories() {
    ...
    for (const p of allProducts) {
      const slug = p.categorySlug || ALL_CAT_SLUG;
      const label = p.categoryLabel || "Others";
      if (!map.has(slug)) map.set(slug, { slug, label, count: 0 });
      map.get(slug).count++;
    }
    const totalCount = allProducts.length;
    categories = [
      { slug: ALL_CAT_SLUG, label: "Zobrazit vše", count: totalCount },
  ...
  function applyCategoryFilter() {
    if (currentCategorySlug === ALL_CAT_SLUG) {
      ...
      filteredProducts = allProducts.filter(
        (p) => p.categorySlug === currentCategorySlug
      );
    }
  }
  ...
  function renderCategoryNav() {
    ...
    nav.innerHTML = categories
      .map((c, idx) => {
        const isActive = c.slug === currentCategorySlug;
        return `
          <a href="#" class="mf-cat-link ${
            isActive ? "active" : ""
          }" data-cat="${c.slug}">
            ${c.label} (${c.count})
          </a>
