...
(async function () {
    ...
    const DATA_URL = "js/data/Combisteel_Pizzeria_Dough_preparation.json";
    ...
    function buildFilterPanel() {
        const groupsHtml = Object.entries(FILTER_CONFIG)
            .map(([groupId, conf]) => {
              ...
              `;
                ...
                // block filter giống structure bạn gửi (Category / Depth / Height ...)
                return `
          <div class="filter-option mb-3 pb-3 border-black border-b" x-data="{ open: false }">
            <div class="filter-options-title flex justify-between items-center cursor-pointer text-normal hover:text-pizza"
                 :class="{ 'pb-4': open }"
                 @click="open = !open">
        ...
        // ❌ BỎ phần "shoppingOptions / catalogue" ở đầu
        filterPanel.innerHTML = groupsHtml;

        // ===== CLICK FILTER (logic giữ nguyên) =====
