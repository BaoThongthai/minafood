...
(async function () {
    ...
    const DATA_URL = "js/casta_equiment/data/casta_ladyQ900.json"; // ← đổi sang file JSON của bạn
    ...
    const normalize = (p) => {
        ...
        const depth = s.depth ?? null;
        const height = s.height ?? null;
        const weight = s.weight ?? null;
        const volume = s.volume ?? null;
        const electric_power = s.electric_power ?? "";
        const electric_intensity = s.electric_intensity ?? "";

        // Dòng 1: kích thước
        const dimStr =
            width || depth || height
                ? `Size (W×D×H): ${[width, depth, height].filter(Boolean).join(" x ")} mm`
                : "";

        // Dòng 2: các thông số còn lại
        const detailParts = [];
        if (weight != null) detailParts.push(`Weight: ${weight} kg`);
        if (volume != null) detailParts.push(`Volume: ${volume} m³`);
        if (electric_power) detailParts.push(`Electric power: ${electric_power}`);
        if (electric_intensity) detailParts.push(`Frequency: ${electric_intensity}`);
        ...
        return {
            ...
            line2: detailStr,
            label: "",
            // CASTA không có giá → để null
            price: null,
            currency: "",
            image: p?.image || "img/placeholder.webp",
            href: p?.url || "#", // chỉ dùng cho email/inquiry, không hiển thị
            sp: null,
            // lưu lại specs để dùng cho popup nếu muốn
            specs: {
                width,
                depth,
                height,
    ...
    const specsHTML = (p) => {
        ...
        if (s.width || s.depth || s.height) {
            rows.push(
                `Size (W×D×H): ${[s.width, s.depth, s.height]
                    .filter(Boolean)
                    .join(" x ")} mm`
            );
        }
        ...
        if (s.electric_power) rows.push(`Electric power: ${s.electric_power}`);
        if (s.electric_intensity)
            rows.push(`Frequency: ${s.electric_intensity}`);
  ...
  `;
    ...
    function openPopup(p) {
        ...
        // Hiển thị FULL specs trong popupDim
        const s = p.specs || {};
        const rows = [];
        ...
        // Dòng dưới chỉ hiển thị Code (không cần url, không có price)
