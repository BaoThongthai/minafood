...
(async function () {
    ...
    const DATA_URL = "js/data/combisteel_buffet_equipment.json";
    ...
    // ====== CATEGORY MAPPING THEO TÊN SẢN PHẨM ======
    // Kết quả cuối cùng:
    //  - Ceramic top units
    //  - Bain-maries
    //  - Refrigerated top units
    //  - Refrigerated units
    //  - Refrigerated display
    //  - Heated ceramic display
    //  - Neutral display
    //  - Overshelf glasses
    //  - Plate and container lifts
    //  - Chung (fallback)
    // ====== CATEGORY MAPPING THEO TÊN SẢN PHẨM ======
    function getCategoryLabel(raw) {
        const name = txt(raw.name).toLowerCase();

        // Ceramic top units: các top ceramic, top unit
        if (name.includes("heated ceramic top") || name.includes("heated top unit")) {
            return "Ceramic top units";
        }

        // Heated ceramic display: display + thanh đèn có ceramic heating
        if (name.includes("heated display") || name.includes("lighting bar with ceramic heating")) {
            return "Heated ceramic display";
        }

        // Bain-maries
        if (name.includes("bain-marie")) {
            return "Bain-maries";
        }

        // Refrigerated top units
        if (name.includes("refrigerated top unit")) {
            return "Refrigerated top units";
        }

        // Refrigerated units
        if (name.includes("refrigerated unit")) {
            return "Refrigerated units";
        }

        // Refrigerated display (trong JSON hiện tại chưa có cái nào, nên count = 0)
        if (name.includes("refrigerated display")) {
            return "Refrigerated display";
        }

        // Neutral display
        if (name.includes("neutral display")) {
            return "Neutral display";
        }

        // Plate and container lifts: plates lift, cutlery unit...
        if (name.includes("plates lift") || name.includes("cutlery unit")) {
            return "Plate and container lifts";
        }

        // Overshelf glasses: curved glass, lighting bar thường, brackets
        if (
            name.includes("curved glass") ||
            (name.includes("lighting bar") && !name.includes("ceramic heating")) ||
            name.includes("brackets for curved glass")
        ) {
            return "Overshelf glasses";
        }

        // Các món khó phân loại: crushed ice bin, sink for ice cream scoop
        if (
            name.includes("crushed ice bin") ||
            name.includes("sink for ice cream scoop") ||
            name.includes("ice bin")
        ) {
            return "Generic";
        }
    ...
    //   specs:{ width, depth, height, material, version, parcel_ready, model_type, voltage, ... } }
  ...
  `;
    ...
    try {
        ...
        console.error("Load buffet equipment failed:", err);
        grid.innerHTML = `
      <div class="col-12">
